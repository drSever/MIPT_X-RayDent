...
class MaskRCNNInference:
    ...
    def __init__(self, model_path, num_classes=32, score_threshold=0.5, 
        ...
        """
        self.cfg = self._setup_config(model_path, num_classes, score_threshold, test_image_size)
    ...
    def _setup_config(self, model_path, num_classes, score_threshold, test_image_size):
        ...
        # Размеры изображений (должны совпадать с обучением !!!)
        cfg.INPUT.MIN_SIZE_TEST = test_image_size
        cfg.INPUT.MAX_SIZE_TEST = test_image_size  # В данном случае изображение ресайзится в квадрат
        
        # Anchor параметры - ДОЛЖНЫ СОВПАДАТЬ с параметрами при обучении!
        # Модель обучена с правильной FPN конфигурацией
