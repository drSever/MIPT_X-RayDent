...
##########################################################################################
# РАЗДЕЛ: ФУНКЦИИ ДЛЯ РАБОТЫ С ПРЕДОБУЧЕННЫМИ МОДЕЛЯМИ
##########################################################################################

### Создание моделей с предобученными энкодерами ###

def create_model(architecture, encoder_name, encoder_weights, in_channels, classes, dropout=0.0):
    """
    Создает модель с предобученным энкодером
    
    Args:
        architecture: 'unet', 'unetplusplus', 'fpn', 'deeplabv3plus'
        encoder_name: название энкодера (resnet34, efficientnet-b3, etc.)
        encoder_weights: 'imagenet', 'ssl', 'swsl' или None
        in_channels: 1 для grayscale, 3 для RGB
        classes: количество классов (33 для зубов)
        dropout: dropout для регуляризации
    
    Returns:
        model: модель с предобученным энкодером
    """
...
def load_pretrained_model(model_path, architecture, encoder_name, num_classes=33, device='cuda'):
    """
    Загрузка обученной модели с предобученным энкодером
    ...
    """
    # Создаем модель с той же архитектурой (без предобученных весов)
    model = create_model(
        architecture=architecture,
        encoder_name=encoder_name,
        encoder_weights=None,  # Не загружаем предобученные веса, т.к. загрузим из чекпоинта
        in_channels=1,
        classes=num_classes
    )
...
### Функции для инференса с предобученными моделями ###

def inference_single_image_pretrained(model_path, image_path, data_yaml_path, transform, 
                                     architecture, encoder_name, output_dir='output', device='cuda'):
    """
    Инференс на одном изображении с предобученной моделью
    """
    # Преобразуем output_dir в Path если это строка
    if isinstance(output_dir, str):
        output_dir = Path(output_dir)
    ...
    # Загружаем модель
    print("Загрузка модели...")
    model = load_pretrained_model(model_path, architecture, encoder_name, num_classes, device)
...
def inference_multiple_images_pretrained(model_path, image_dir, data_yaml_path, transform,
                                        architecture, encoder_name, output_dir='output', device='cuda'):
    """
    Инференс на нескольких изображениях в директории с предобученной моделью
    """
    # Преобразуем пути в Path объекты
    image_dir = Path(image_dir)
    if isinstance(output_dir, str):
    ...
    for image_path in image_files:
        ...
        try:
            ...
            mask, result = inference_single_image_pretrained(
                model_path, image_path, data_yaml_path, transform,
                architecture, encoder_name, img_output_dir, device
            )
            masks.append(mask)
            results.append(result)
            print(f"✓ Успешно обработано: {image_path.name}")
