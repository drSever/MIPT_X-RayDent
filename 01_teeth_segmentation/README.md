...
1. У исследователей накоплен достаточно большой опыт сегментации зубов на ортопантомограммах (ОПТГ), на arxiv.org и PubMed изначально найдено почти 100 статей, посвященных данному вопросу за последние 5 лет.
2. Классические архитектуры (YOLO, U-Net, MaskRCNN) до сих пор демонстрируют удовлетворительные результаты и до сих пор используются в решении разных задач сегментации зубов.
3. Новые современные архитектуры (SAM, трансформеры, KAN) и гибридные модели дают еще больший прирост качества сегментации, по заявлению авторов исследований.
4. Во время обучения авторы применяли аугментаии с целью увеличения датасета и снижения переобучения модели.
5. Большинство моделей обучались с использованием оптимизаторов Adam или SGD.
6. В некоторых случаях применялся Dice Loss для решения проблемы дисбаланса классов (когда зубы занимают малую часть снимка).
