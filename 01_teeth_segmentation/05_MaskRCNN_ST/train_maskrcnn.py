######################################################################
# Обучение Mask R-CNN Swin Transformer для сегментации зубов
######################################################################
...
class PeriodicEvaluator(HookBase):
    ...
    def __init__(self, eval_period, model, val_loader, val_dataset_name,
                 output_dir, history, best_model_saver, val_loss_loader=None,
                 val_loss_batches=20):
        ...
        self.val_loss_batches = val_loss_batches  # сколько батчей брать для val losses
    ...
    def after_step(self):
        ...
        if next_iter % self.eval_period == 0:
            ...
            if self.val_loss_loader is not None:
                self.model.train()
                num_batches = 0
                with torch.no_grad():
                    for batch in self.val_loss_loader:
                        ...
                        val_losses['total_loss']   += total.item()
                        val_losses['loss_cls']     += loss_dict.get('loss_cls',     torch.tensor(0.0)).item()
                        val_losses['loss_box_reg'] += loss_dict.get('loss_box_reg', torch.tensor(0.0)).item()
                        val_losses['loss_mask']    += loss_dict.get('loss_mask',    torch.tensor(0.0)).item()
                        val_losses['loss_rpn_cls'] += loss_dict.get('loss_rpn_cls', torch.tensor(0.0)).item()
                        val_losses['loss_rpn_loc'] += loss_dict.get('loss_rpn_loc', torch.tensor(0.0)).item()
                        num_batches += 1
                        if num_batches >= self.val_loss_batches:
                            break
                if num_batches > 0:
                    for k in val_losses:
            ...
            metrics = {
                ...
                'val_loss_mask':    val_losses['loss_mask'],
                'val_loss_rpn_cls': val_losses['loss_rpn_cls'],
                'val_loss_rpn_loc': val_losses['loss_rpn_loc'],
            }
            ...
            print(f"    Cls:      {val_losses['loss_cls']:.4f}")
            print(f"    Box Reg:  {val_losses['loss_box_reg']:.4f}")
            print(f"    Mask:     {val_losses['loss_mask']:.4f}")
            if 'segm' in results:
            ...
            # Возвращаем модель в train() — inference_on_dataset переключает в eval()
            self.model.train()
...
class MaskRCNNTrainer(DefaultTrainer):
    ...
    def __init__(self, cfg, val_loader, val_dataset_name, history, class_weights=None, use_augmentations=True):
        ...
        if self.class_weights is not None:
            is_resume = cfg.MODEL.WEIGHTS and not cfg.MODEL.WEIGHTS.startswith("detectron2://") \
                        and not "model_zoo" in cfg.MODEL.WEIGHTS \
                        and cfg.MODEL.WEIGHTS != ""
            if is_resume:
                print(f"\n{'!'*70}")
                print(f"ВНИМАНИЕ: продолжение обучения с class weights.")
                print(f"  Class weights применяются к loss функции поверх загруженного чекпоинта.")
                print(f"  Веса модели и состояние оптимизатора загружены корректно.")
                print(f"  Убедитесь что class_weight_method совпадает с предыдущим запуском,")
                print(f"  иначе баланс классов изменится относительно сохранённого состояния.")
                print(f"{'!'*70}\n")
            self._apply_class_weights()
    
    @classmethod
    def build_optimizer(cls, cfg, model):
        """Используем AdamW вместо SGD — оптимально для Swin Transformer"""
        params = []
        for key, value in model.named_parameters():
            ...
            # Не применяем weight decay к bias и norm слоям
            if "bias" in key or "norm" in key or "bn" in key:
                weight_decay = 0.0
    ...
    def build_hooks(self):
        ...
        val_loss_loader = build_detection_train_loader(
            dataset=DatasetCatalog.get("teeth_val"),
...
def setup_config(num_classes, output_dir, resume_from=None, eval_period=1000,
                 max_iter=10000, batch_size=2, base_lr=0.0001,
    ...
    """
    Настройка конфигурации Mask R-CNN + Swin Transformer backbone
    ...
    Args:
        ...
        base_lr: начальный learning rate (для Swin рекомендуется 0.0001, не 0.00025)
        ...
        swin_model_name: имя модели timm.
                        Варианты:
                          "swin_tiny_patch4_window7_224"   (~28M params, быстрее)
                          "swin_small_patch4_window7_224"  (~50M params)
                          "swin_base_patch4_window7_224"   (~88M params, рекомендуется)
                          "swin_large_patch4_window7_224"  (~197M params, нужно много VRAM)
        swin_pretrained: загружать ли ImageNet-21k pretrained веса через timm
    """
    # Регистрируем Swin backbone (импорт регистрирует его в BACKBONE_REGISTRY)
    import swin_backbone  # noqa: F401

    cfg = get_cfg()

    # Базовый конфиг берём от Mask R-CNN R50-FPN — он совместим с любым FPN backbone
    cfg.merge_from_file(model_zoo.get_config_file(
        "COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml"
    ))

    # Добавляем кастомные ключи для Swin
    cfg.MODEL.SWIN = CN()
    cfg.MODEL.SWIN.MODEL_NAME = swin_model_name
    cfg.MODEL.SWIN.PRETRAINED = swin_pretrained

    # Подключаем наш backbone
    cfg.MODEL.BACKBONE.NAME = "build_swin_fpn_backbone"
    cfg.MODEL.BACKBONE.FREEZE_AT = 0  # не замораживаем (fine-tune всего backbone)

    # FPN выходные каналы (стандарт 256)
    cfg.MODEL.FPN.OUT_CHANNELS = 256

    # Pixel mean/std для ImageNet (Swin обучен на ImageNet)
    cfg.MODEL.PIXEL_MEAN = [123.675, 116.280, 103.530]  # RGB
    cfg.MODEL.PIXEL_STD = [58.395, 57.120, 57.375]

    # Веса: если resume — грузим чекпоинт, иначе timm сам загрузит pretrained
    if resume_from is None:
        cfg.MODEL.WEIGHTS = ""  # timm загружает веса сам при swin_pretrained=True
    else:
    ...
    # AdamW лучше подходит для Transformer-based backbone
    # Gradient clipping через norm (единственный валидный тип в detectron2)
...
def train_model(
    ...
    Args:
        ...
        base_lr: начальный learning rate (для Swin рекомендуется 0.0001)
        ...
        test_image_size: размер для валидации и inference (int)
        swin_model_name: имя модели timm (swin_tiny/small/base/large_patch4_window7_224)
        swin_pretrained: загружать ли ImageNet-21k pretrained веса
    """
    ...
    print(f"\nМодель:")
    print(f"  Архитектура: Mask R-CNN + Swin Transformer FPN")
    print(f"  Backbone: {cfg.MODEL.SWIN.MODEL_NAME}")
    print(f"  Pretrained: {'ImageNet-21k (через timm)' if cfg.MODEL.SWIN.PRETRAINED else 'Нет'}")
...
if __name__ == "__main__":
    ...
    #   "swin_base_patch4_window7_224"   — ~88M params, ~14GB VRAM (рекомендуется)
    #   "swin_large_patch4_window7_224"  — ~197M params, ~24GB VRAM 
    SWIN_MODEL_NAME = "swin_base_patch4_window7_224"
    SWIN_PRETRAINED = True  # загружать pretrained веса через timm (рекомендуется)

    # Параметры обучения
    MAX_ITER = 10000           # Количество итераций
    BATCH_SIZE = 2             # Размер батча
    BASE_LR = 0.0001           # LR для Swin (ниже чем для ResNet/ResNeXt)
